...
Project FileLoader::loadProject(string source)
{
	...
	/**if (openFile) //si le fichier existe
	{
		...
		City c(height, width); //cr�ation de la city
		...
		for (size_t i = 0; i < nbOfBuildingProjects; i++) { //Lecture des buildings
			...
			Building b(i, rowNum, columnNum, occupiedCells);//cr�ation du building
			vect.push_back(b); //ajout du b�timent au vecteur

			if (projectType == "R") //Remplissage des index en fonction du type de b�timent
